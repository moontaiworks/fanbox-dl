...
impl AssetDownloader {
    ...
    pub async fn download(&self, published_datetime: &str, root_directory: &Path, relative_path: &Path, url: &str) -> Result<AssetDownloadResult> {
        ...
        let partial_bytes = tokio::fs::metadata(&temporary_path).await.map(|meta| meta.len()).unwrap_or(0);
        let mut request = HttpRequest::get(url.parse()?);
        if partial_bytes > 0 {
            request.headers.insert("Range", format!("bytes={partial_bytes}-").parse()?);
        }
        let http = Arc::clone(&self.http);
        let response = self.scheduler.fetch(move || {
        ...
        let timestamp = response.headers.get("Last-Modified").and_then(|value| value.to_str().ok()).and_then(|value| chrono::DateTime::parse_from_rfc2822(value).ok().map(|date| date.timestamp())).unwrap_or(chrono::DateTime::parse_from_rfc3339(published_datetime)?.timestamp());
        filetime::set_file_times(&destination, FileTime::from_unix_time(timestamp, 0), FileTime::from_unix_time(timestamp, 0))?;
...
mod tests {
    ...
    async fn resumes_partial_asset_with_range_request() -> Result<()> {
        ...
        headers.insert("Last-Modified", "Wed, 27 May 2026 12:17:41 GMT".parse().unwrap());
        let http = RecordingHttpClient::new(vec![Ok(HttpResponse { body: b"def".to_vec(), headers, status: 206, status_text: "Partial Content".into() })]);
        let downloader = AssetDownloader::new(Arc::new(http.clone()), RequestScheduler::new(1));
        let result = downloader.download("2026-05-27T21:17:41+09:00", directory.path(), Path::new("assets/image.png"), "https://example.test/image.png").await?;
        assert_eq!(http.requests.lock().unwrap()[0].headers.get("Range").unwrap(), "bytes=3-");
    ...
    async fn restarts_partial_asset_when_server_returns_full_response() -> Result<()> {
        ...
        let http = RecordingHttpClient::new(vec![Ok(HttpResponse { body: b"new".to_vec(), headers: HeaderMap::new(), status: 200, status_text: "OK".into() })]);
        let downloader = AssetDownloader::new(Arc::new(http), RequestScheduler::new(1));
        downloader.download("2026-05-27T21:17:41+09:00", directory.path(), Path::new("asset.bin"), "https://example.test/asset.bin").await?;
    ...
    async fn rejects_asset_path_over_budget() {
        ...
        let http = RecordingHttpClient::new(vec![Ok(HttpResponse { body: b"data".to_vec(), headers: HeaderMap::new(), status: 200, status_text: "OK".into() })]);
        let downloader = AssetDownloader::new(Arc::new(http), RequestScheduler::new(1));
        assert!(downloader.download("2026-05-27T21:17:41+09:00", directory.path(), Path::new(&format!("{}.bin", "x".repeat(300))), "https://example.test/asset.bin").await.is_err());
    }
}
