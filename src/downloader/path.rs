...
pub fn assert_path_budget(path: &Path, max_bytes: usize) -> anyhow::Result<()> {
    if path.to_string_lossy().as_bytes().len() > max_bytes {
        anyhow::bail!("Path exceeds {max_bytes} byte path budget: {}", path.display());
    }
    Ok(())
}

pub fn create_creator_directory_name(creator_id: &str, output_directory: &Path) -> anyhow::Result<String> {
...
pub fn create_post_directory_name(id: &str, published_datetime: &str, title: &str, parent_directory: Option<&Path>) -> anyhow::Result<String> {
    ...
    let options = SanitizePathComponentOptions {
        ...
        reserve_bytes: Some(prefix.len() + std::path::MAIN_SEPARATOR_STR.len() + "metadata.json".len()),
        suffix: None,
    };
...
pub fn sanitize_path_component(value: &str, options: SanitizePathComponentOptions) -> String {
    ...
    format!("{}{}", truncate_utf8(&sanitized, max_bytes.saturating_sub(suffix_bytes)), suffix)
}

pub fn sanitize_path_component_for_directory(value: &str, directory: &Path, mut options: SanitizePathComponentOptions) -> anyhow::Result<String> {
    let available_bytes = DEFAULT_PATH_BYTES
        .saturating_sub(directory.canonicalize().unwrap_or_else(|_| PathBuf::from(directory)).to_string_lossy().as_bytes().len())
    ...
    if available_bytes <= options.suffix.as_ref().map_or(0, String::len) {
        anyhow::bail!("Directory leaves no room in path budget: {}", directory.display());
    }
    options.max_bytes = Some(options.max_bytes.unwrap_or(DEFAULT_COMPONENT_BYTES).min(available_bytes));
...
fn is_reserved_name(value: &str) -> bool {
    ...
    matches!(uppercase.as_str(), "AUX" | "CON" | "NUL" | "PRN")
        || uppercase.starts_with("COM") && uppercase[3..].chars().all(|ch| ('1'..='9').contains(&ch))
        || uppercase.starts_with("LPT") && uppercase[3..].chars().all(|ch| ('1'..='9').contains(&ch))
}
...
fn starts_with_reserved_prefix(value: &str) -> bool {
    ...
    ["AUX", "CON", "NUL", "PRN", "COM1", "COM2", "COM3", "COM4", "COM5", "COM6", "COM7", "COM8", "COM9", "LPT1", "LPT2", "LPT3", "LPT4", "LPT5", "LPT6", "LPT7", "LPT8", "LPT9"]
        .into_iter()
        .any(|name| upper == name || upper.starts_with(&format!("{name}.")) || upper.starts_with(&format!("{name} ")) || upper.starts_with(&format!("{name}<")) || upper.starts_with(&format!("{name}>")) || upper.starts_with(&format!("{name}:")) || upper.starts_with(&format!("{name}\"")) || upper.starts_with(&format!("{name}/")) || upper.starts_with(&format!("{name}\\")) || upper.starts_with(&format!("{name}|")) || upper.starts_with(&format!("{name}?")) || upper.starts_with(&format!("{name}*")))
}
...
mod tests {
    ...
    use super::{assert_path_budget, create_creator_directory_name, create_post_directory_name, sanitize_path_component, sanitize_path_component_for_directory, SanitizePathComponentOptions};

    #[test]
    fn removes_invalid_characters_and_protects_reserved_names() {
        assert_eq!(sanitize_path_component("CON<>:\"/\\|?*. ", SanitizePathComponentOptions::default()), "_CON_________");
    }
    ...
    fn truncates_utf8_without_breaking_suffix() {
        let value = sanitize_path_component(
            &"界".repeat(100),
            SanitizePathComponentOptions { max_bytes: Some(20), suffix: Some("_123.png".into()), reserve_bytes: None },
        );
        assert!(value.len() <= 20);
    ...
    fn truncates_component_to_fit_directory() {
        ...
        let value = sanitize_path_component_for_directory(
            &"x".repeat(300),
            directory,
            SanitizePathComponentOptions { suffix: Some(".png".into()), ..SanitizePathComponentOptions::default() },
        )
        .unwrap();
        assert!(directory.join(&value).to_string_lossy().as_bytes().len() <= 240);
        assert!(value.ends_with(".png"));
    }

    #[test]
    fn creates_safe_creator_directory() {
        assert_eq!(create_creator_directory_name("../creator", Path::new("/tmp")).unwrap(), ".._creator");
    }

    #[test]
    fn creates_post_directory_name() {
        assert_eq!(create_post_directory_name("123", "2026-05-27T21:17:41+09:00", "A title", None).unwrap(), "2026-05-27_123_A title");
    }
