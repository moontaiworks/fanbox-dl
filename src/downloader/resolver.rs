...
pub async fn resolve_creator_ids(
    client: &FanboxClient,
    ...
    if options.supporting {
        for plan in client.list_supporting_plans().await? {
            creator_ids.insert(plan.creator_id);
        }
    }
