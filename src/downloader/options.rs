...
pub fn parse_download_options<I, T>(
    args: I,
    ...
    T: Into<std::ffi::OsString> + Clone,
{
    ...
    let command = Command::new("download")
        ...
        .arg(
            Arg::new("output")
                .long("output")
                .num_args(1)
                .default_value("fanbox-downloads"),
        )
        .arg(
            Arg::new("dry-run")
                .long("dry-run")
        ...
        .arg(
            Arg::new("rate-limit-pause-ms")
                .long("rate-limit-pause-ms")
                .num_args(1)
                .default_value("60000"),
        )
        .arg(
    ...
    let creator_ids = matches
        .get_many::<String>("creator")
    ...
    Ok(DownloadOptions {
        concurrency: parse_positive_integer(
            "concurrency",
            matches.get_one::<String>("concurrency").unwrap(),
        )?,
        cookie,
        creator_ids,
        dry_run: matches.get_flag("dry-run"),
        following: matches.get_flag("following"),
        ignore_creator_ids: matches
            .get_many::<String>("ignore-creator")
            .map(|values| values.cloned().collect::<Vec<_>>())
            .unwrap_or_default(),
        log_format,
        max_retries: parse_non_negative_integer(
            "max-retries",
            matches.get_one::<String>("max-retries").unwrap(),
        )?,
        output: PathBuf::from(matches.get_one::<String>("output").unwrap()),
        rate_limit_pause_ms: parse_non_negative_integer(
            "rate-limit-pause-ms",
            matches.get_one::<String>("rate-limit-pause-ms").unwrap(),
        )? as u64,
        request_interval_ms: parse_non_negative_integer(
            "request-interval-ms",
            matches.get_one::<String>("request-interval-ms").unwrap(),
        )? as u64,
        supporting: matches.get_flag("supporting"),
        user_agent: matches
            .get_one::<String>("user-agent")
            .cloned()
            .or_else(|| env.get("FANBOX_USER_AGENT").cloned()),
...
mod tests {
    ...
    fn parses_repeated_selectors_and_defaults() {
        let options = parse_download_options(
            [
                "download",
                "--creator",
                "alpha",
    ...
    fn loads_netscape_cookies() {
        ...
        assert_eq!(
            options.cookie,
            Some("cf_clearance=clearance; FANBOXSESSID=session".into())
        );
    }
